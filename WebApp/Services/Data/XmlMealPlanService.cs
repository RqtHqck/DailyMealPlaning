...
public class XmlMealPlanService
{
    ...
    public void SaveMealPlanToFile(MealsPlan plan, string filePath)
    {
        var doc = new XDocument(
            new XElement("Db", new XElement("MealsPlan", plan.MealItems.Select(m =>
                new XElement("MealItem",
                    new XAttribute("name", m.Name),
                    new XAttribute("type", m.Type.ToString()),
        ...
        doc.Save(filePath); // Save to the specified file location
    }
}